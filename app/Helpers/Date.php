...
class Date
{
	 ...
	 * @return string
	 */
	public static function format($value, $datetime = false)
	{
		if ($value instanceof Carbon) {
			$dateFormat = self::getAppDateFormat($datetime);
	...
	public static function formatFormNow($value)
	{
		...
		$formattedDate = self::format($value, true);
	 ...
	 * @return \Illuminate\Config\Repository|\Illuminate\Contracts\Foundation\Application|mixed
	 */
	private static function getAppDateFormat($datetime = false)
	{
		$adminDateFormat = $datetime
		...
		$langFrontDateFormat = $datetime ? config('lang.datetime_format') : config('lang.date_format');
		$frontDateFormat = !empty($langFrontDateFormat) ? $langFrontDateFormat : $adminDateFormat;
		
		$countryFrontDateFormat = $datetime ? config('country.datetime_format') : config('country.date_format');
		...
		if (empty($dateFormat)) {
			$dateFormat = $datetime ? config('larapen.core.datetimeFormat.default') : config('larapen.core.dateFormat.default');
		}
