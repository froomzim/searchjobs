...
trait DbTrait
{
	...
	private function fixAdminUserPermissions()
	{
		$firstUser = DB::table('users')->orderBy('id', 'ASC')->first();
		if (!empty($firstUser)) {
			$brokenMasterAdmin = DB::table('users')->where('id', $firstUser->id)->whereNull('is_admin')->first();
			if (!empty($brokenMasterAdmin)) {
				DB::table('users')->where('id', '!=', $brokenMasterAdmin->id)->update(['is_admin' => 0]);
				DB::table('users')->where('id', $brokenMasterAdmin->id)->update(['is_admin' => 1]);
