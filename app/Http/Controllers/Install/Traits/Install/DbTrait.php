...
trait DbTrait
{
	...
	private function submitDatabaseImport(Request $request, $siteInfo, $database)
	{
		...
		try {
			$countCountries = DB::table('countries')->count(); // Latest seeder run
		...
		try {
			...
			if (is_array($modelFiles) && count($modelFiles) > 0) {
				foreach ($modelFiles as $filePath) {
					$table = $this->getModelTableName($filePath);
	 ...
	 */
	private function getModelTableName($fileFullPath, $tablesPrefix = null)
	{
