...
trait MigrationsTrait
{
	...
	protected function runSiteInfoSeeder($siteInfo)
	{
		try {
			...
			DB::table('countries')->where('code', $siteInfo['default_country'])->update(['active' => 1]);
			...
			DB::table('settings')->where('key', 'app')->update(['value' => json_encode($app)]);
			
			// Geo Location
			$geoLocation = [
				'default_country_code' => isset($siteInfo['default_country']) ? $siteInfo['default_country'] : '',
			];
			DB::table('settings')->where('key', 'geo_location')->update(['value' => json_encode($geoLocation)]);
			...
			DB::table('settings')->where('key', 'mail')->update(['value' => json_encode($mail)]);
